...
static void process_receive_notification(void * p_event_data, uint16_t event_size) {
	...
	if(decode_status == 0) {
		NRF_LOG_INFO("REQUEST_HANDLER: Error decoding\n");
		finish_error();
		return;
	}
	NRF_LOG_INFO("REQUEST_HANDLER: Decoded successfully\n");
	
	if(istream.bytes_read < len) {
		NRF_LOG_INFO("REQUEST_HANDLER: Warning decoding: %u bytes have not been read.\n", len - istream.bytes_read);
	}
...
static void send_response(void * p_event_data, uint16_t event_size) {
	...
	if(encode_status == 0) {
		NRF_LOG_INFO("REQUEST_HANDLER: Error encoding response, len: %u!\n", len);
		finish_error();
		return;
	}
...
static void start_microphone_request_handler(void * p_event_data, uint16_t event_size) {
	...
	NRF_LOG_INFO("REQUEST_HANDLER: Start microphone with timeout: %u, period ms %u\n", timeout, period_ms);
	
//	ret_code_t ret = sampling_start_microphone(timeout*60*1000, period_ms, 0);
//	NRF_LOG_INFO("REQUEST_HANDLER: Ret sampling_start_microphone: %d\n\r", ret);
//
//	if(ret == NRF_SUCCESS) {
...
static void start_accelerometer_request_handler(void * p_event_data, uint16_t event_size) {
	...
	NRF_LOG_INFO("REQUEST_HANDLER: Start accelerometer with timeout: %u, operating_mode: %u, full_scale: %u, datarate: %u, fifo_sampling_period_ms: %u\n", timeout, operating_mode, full_scale, datarate, fifo_sampling_period_ms);
	
//	ret_code_t ret = sampling_start_accelerometer(timeout*60*1000, operating_mode, full_scale, datarate, fifo_sampling_period_ms, 0);
//	NRF_LOG_INFO("REQUEST_HANDLER: Ret sampling_start_accelerometer: %d\n\r", ret);
//
//	if(ret == NRF_SUCCESS) {
